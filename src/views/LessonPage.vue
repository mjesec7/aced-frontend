<template>
  <div class="lesson-page">
    ...
    <!-- Premium Content Modal (Paywall) -->
    <Teleport to="body">
      <div v-if="showPaywallModal" class="fixed inset-0 z-[1000] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl">
          <!-- Gradient Header -->
          <div class="bg-gradient-to-br from-violet-500 to-indigo-600 p-6 text-center">
            ...
            <h3 class="text-xl font-bold text-white mb-1">{{ $t('lessonModals.premiumContent.title') }}</h3>
            <p class="text-white/80 text-sm">{{ $t('lessonModals.premiumContent.description') }}</p>
          </div>
          ...
          <div class="p-5">
            <div class="space-y-2.5 mb-5">
              <div class="flex items-center gap-3 text-sm text-slate-600">
                ...
                <span>{{ $t('lessonModals.premiumContent.benefit1') }}</span>
              </div>
              <div class="flex items-center gap-3 text-sm text-slate-600">
                <div class="w-5 h-5 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0">
                  <svg class="w-3 h-3 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                ...
                <span>{{ $t('lessonModals.premiumContent.benefit2') }}</span>
              </div>
              <div class="flex items-center gap-3 text-sm text-slate-600">
                <div class="w-5 h-5 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0">
                  <svg class="w-3 h-3 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                ...
                <span>{{ $t('lessonModals.premiumContent.benefit3') }}</span>
              </div>
            </div>
            ...
            <div class="space-y-2">
              <button 
                @click="$router.push('/pay/start')" 
                class="w-full py-3 bg-gradient-to-r from-violet-500 to-indigo-600 text-white font-semibold rounded-xl hover:from-violet-600 hover:to-indigo-700 transition-all shadow-lg shadow-violet-200"
              >
                {{ $t('lessonModals.premiumContent.getPremium') }}
              </button>
              <button 
                @click="$router.push('/pricing')" 
                class="w-full py-2.5 text-violet-600 font-medium hover:text-violet-700 hover:bg-violet-50 rounded-xl transition-colors"
              >
                {{ $t('lessonModals.premiumContent.viewPlans') }}
              </button>
              <button 
                @click="handleReturnToCatalogue" 
                class="w-full py-2 text-slate-400 text-sm hover:text-slate-600 transition-colors"
              >
                {{ $t('lessonModals.premiumContent.maybeLater') }}
              </button>
            </div>
          </div>
        </div>
    ...
    <Teleport to="body">
      <div v-if="showExitModal" class="fixed inset-0 z-[1000] bg-black/40 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-xs p-6 text-center">
          <h3 class="text-lg font-bold text-slate-900 mb-2">{{ $t('lessonModals.exitLesson.title') }}</h3>
          <p class="text-slate-500 text-sm mb-6">{{ $t('lessonModals.exitLesson.description') }}</p>
          <div class="space-y-2">
      ...
      <div v-if="showProblemReportModal" class="fixed inset-0 z-[1000] bg-black/40 flex items-center justify-center p-4" @click.self="closeProblemReportModal">
        <div class="bg-white rounded-2xl w-full max-w-md max-h-[85vh] overflow-hidden flex flex-col">
          <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-900">{{ $t('lessonModals.problemReport.title') }}</h3>
          ...
          <div class="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1.5">{{ $t('lessonModals.problemReport.selectType') }}</label>
              <select v-model="problemType" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent">
                <option value="">{{ $t('lessonModals.problemReport.selectType') }}</option>
                <option value="content">{{ $t('lessonModals.problemReport.contentError') }}</option>
                <option value="technical">{{ $t('lessonModals.problemReport.technicalIssue') }}</option>
                <option value="exercise">{{ $t('lessonModals.problemReport.exerciseProblem') }}</option>
                <option value="other">{{ $t('lessonModals.problemReport.otherIssue') }}</option>
              </select>
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1.5">{{ $t('lessonModals.problemReport.description') }} *</label>
              <textarea
                v-model="problemDescription"
                rows="3"
                :placeholder="$t('lessonModals.problemReport.descriptionPlaceholder')"
              ...
              ></textarea>
              <p v-if="showValidationError && !problemDescription.trim()" class="mt-1 text-xs text-red-500">{{ $t('lessonModals.problemReport.descriptionRequired') }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1.5">{{ $t('lessonModals.problemReport.contact') }}</label>
              <input
                type="text"
                v-model="contactInfo"
                :placeholder="$t('lessonModals.problemReport.contactPlaceholder')"
          ...
          <div class="px-5 py-4 border-t border-slate-100 flex gap-3">
            <button @click="closeProblemReportModal" class="flex-1 py-3 text-slate-600 font-medium rounded-xl hover:bg-slate-100 transition-colors">
              {{ $t('lessonModals.problemReport.cancel') }}
            </button>
            <button
            ...
            >
              {{ isSubmitting ? $t('lessonModals.problemReport.sending') : $t('lessonModals.problemReport.send') }}
            </button>
          </div>
        </div>
      </div>
