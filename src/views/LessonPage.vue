<template>
  <div class="lesson-page">
    ...
    <button
      ...
      :title="lexiIsPlaying ? 'Stop Lexi' : lexiIsListening ? 'Listening...' : 'Click: Listen to Lexi | Right-click: Speak to Lexi'"
    >
  ...
  setup() {
    ...
    watch(() => lessonOrchestrator.currentStep.value, (newStep, oldStep) => {
      if (newStep?.id !== oldStep?.id) {
        ...
        initializationTracker.value = { currentExerciseId: null, initialized: false }
        lexi.stop() // Stop Lexi when changing steps
      }
    ...
    const handleLexiSpeak = async () => {
      ...
      // Extract text based on step type
      if (['explanation', 'reading', 'example'].includes(step.type)) {
        textToSpeak = step.data?.content || 
                      step.data?.text || 
                      step.content?.text || 
        ...
        if (word) {
          textToSpeak = `${word.term}. ${word.definition}`
        }
      } else if (step.type === 'game') {
        textToSpeak = step.instructions || step.description || 'Complete the game to continue.'
      }
      ...
      if (textToSpeak && textToSpeak.length > 5) {
        ...
        console.log('No text to speak for this step')
      }
    }
    ...
    const handleLexiListen = async () => {
      ...
      if (userSaid && userSaid.trim()) {
        ...
        // Send to AI chat
        if (explanation.aiChatInput) {
          explanation.aiChatInput.value = userSaid
        }

        if (explanation.sendAIMessage) {
        ...
        // Read AI response with Lexi
        if (explanation.aiChatHistory?.value?.length > 0) {
          const lastAIMessage = explanation.aiChatHistory.value
            .filter(msg => msg.type === 'ai')
            .pop()
          
          if (lastAIMessage?.content) {
            setTimeout(async () => {
              await lexi.speak(lastAIMessage.content)
            }, 500)
          }
        }
      }
