    ...
    async handlePaymentSuccess(paymentData) {
      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          const eventData = {
            source: paymentData.source || 'payment',
            transactionId: this.transactionId,
            plan: statusResult?.status || this.finalPlan,
            timestamp: Date.now()
          };
          window.dispatchEvent(new CustomEvent('userStatusChanged', { detail: eventData }));
          window.dispatchEvent(new CustomEvent('subscriptionUpdated', { detail: eventData }));
        ...
        // ✅ Send inbox message with payment confirmation
        try {
          const { sendPaymentConfirmationMessage } = await import('@/api/inbox');
          const now = new Date();
          const durationDays = this.selectedDuration === 1 ? 30 : this.selectedDuration === 3 ? 90 : 180;
          const endDate = new Date(now.getTime() + durationDays * 24 * 60 * 60 * 1000);

          await sendPaymentConfirmationMessage({
            userId: this.finalUserId,
            amount: this.finalAmount,
            plan: this.finalPlan || 'pro',
            duration: this.selectedDuration,
            transactionId: this.transactionId,
            paymentMethod: this.promoApplied ? 'Promo Code' : (this.paymentProvider === 'payme' ? 'Payme' : 'Bank Card'),
            startDate: now.toISOString(),
            endDate: endDate.toISOString(),
            promoCode: this.promoApplied ? this.promoCodeInput : null,
        ...
        // ✅ AUTO-REDIRECT to settings after 3 seconds so user can see their new status
        setTimeout(() => {
