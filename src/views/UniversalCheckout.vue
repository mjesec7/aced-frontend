  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50/30 flex items-center justify-center p-4 md:p-8">
    <div class="bg-white rounded-2xl shadow-xl shadow-indigo-500/5 max-w-2xl w-full overflow-hidden">
      ...
      <div v-if="paymentStatus === 'success'" class="p-8 md:p-12 text-center">
        ...
        <p class="text-slate-600 mb-8">Your {{ planName }} subscription has been activated</p>
      ...
      <div v-else-if="paymentStatus === 'failed'" class="p-8 md:p-12 text-center">
        ...
        <p class="text-slate-600 mb-8">{{ failureReason || "We couldn't process your payment. Please try again." }}</p>
      ...
      <div v-else-if="paymentStatus === 'cancelled'" class="p-8 md:p-12 text-center">
        ...
        <h1 class="text-2xl font-bold text-slate-900 mb-2">Payment Cancelled</h1>
        <p class="text-slate-600 mb-8">You cancelled the payment. Your subscription remains unchanged.</p>
      ...
      <div v-else-if="error" class="p-8 md:p-12 text-center">
        ...
        <h2 class="text-xl font-semibold text-slate-900 mb-2">Unable to Process Payment</h2>
      ...
      <div v-else class="p-6 md:p-8">
        ...
        <div class="bg-slate-50 rounded-xl p-5 mb-6">
          <h3 class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-4">Payment Details</h3>
          <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
              <span class="text-xs text-slate-500 uppercase">Account</span>
              <p class="font-semibold text-slate-900 truncate">{{ finalUserName || 'Not specified' }}</p>
            </div>
            <div>
              <span class="text-xs text-slate-500 uppercase">Email</span>
              <p class="font-semibold text-slate-900 truncate">{{ finalUserEmail || 'Not specified' }}</p>
            </div>
            <div>
              <span class="text-xs text-slate-500 uppercase">Current Plan</span>
              <span class="inline-block px-3 py-1 bg-emerald-100 text-emerald-700 text-sm font-semibold rounded-lg">{{ currentPlan || 'Free' }}</span>
            </div>
            <div v-if="finalPlan">
              <span class="text-xs text-slate-500 uppercase">Upgrading To</span>
        ...
        <!-- Payment Methods -->
        <div class="mb-6">
          <h3 class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-4">Payment Method</h3>
          <div class="space-y-3">
            <label 
              v-for="(provider, key) in providers" :key="key"
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <span v-if="!provider.enabled" class="text-xs text-amber-600 font-medium block">Coming Soon</span>
              </div>
        ...
        <div class="mb-6">
          ...
          <div class="grid grid-cols-3 gap-3">
            ...
            <label :class="['relative p-4 border-2 rounded-xl cursor-pointer transition-all text-center', selectedDuration === 1 ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200 hover:border-slate-300 bg-white']" @click="selectedDuration = 1">
              ...
              <span class="block text-xs text-slate-500 uppercase">Month</span>
            ...
            <label :class="['relative p-4 border-2 rounded-xl cursor-pointer transition-all text-center', selectedDuration === 3 ? 'border-purple-500 bg-purple-50' : 'border-purple-200 hover:border-purple-300 bg-white']" @click="selectedDuration = 3">
              <div class="absolute -top-2.5 left-1/2 -translate-x-1/2 px-2 py-0.5 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-[10px] font-bold rounded-full">POPULAR</div>
              <input type="radio" v-model="selectedDuration" :value="3" class="hidden"/>
              <span class="block text-xl font-bold text-slate-900">3</span>
              <span class="block text-xs text-slate-500 uppercase">Months</span>
              <span class="block text-lg font-semibold text-purple-600 mt-2">675,000</span>
              <span class="text-xs text-slate-400">UZS</span>
              <span class="block text-[10px] text-emerald-500 font-medium">Save 10%</span>
            ...
            <label :class="['relative p-4 border-2 rounded-xl cursor-pointer transition-all text-center', selectedDuration === 6 ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 hover:border-slate-300 bg-white']" @click="selectedDuration = 6">
              ...
              <span class="text-xs text-slate-400">UZS</span>
              <span class="block text-[10px] text-emerald-500 font-medium">Save 20%</span>
        ...
        <div class="mb-6">
          ...
          <div class="flex gap-2">
            <div class="flex-1 relative">
              <input v-model="promoCodeInput" type="text" placeholder="Enter promo code" :disabled="promoApplying"
        ...
        <div class="flex items-center justify-center gap-2 mt-4 text-slate-400 text-sm">
          ...
          <span>Secure Payment • Encrypted & Protected</span>
        </div>
      </div>
      ...
      <div v-if="showOtpModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="closeOtpModal">
        <div class="bg-white rounded-2xl p-6 max-w-sm w-full">
          <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-bold text-slate-900">Verify Payment</h3>
          ...
          <p class="text-slate-600 text-sm mb-6">Enter the 6-digit code sent to your phone</p>
          ...
          <div class="flex gap-3">
            <button @click="closeOtpModal" :disabled="processing" class="flex-1 py-3 border-2 border-slate-200 text-slate-700 font-semibold rounded-xl hover:bg-slate-50 transition-all">Cancel</button>
