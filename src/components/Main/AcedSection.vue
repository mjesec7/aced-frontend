...
watch: {
displayedCourses(newVal, oldVal) {
  console.log('displayedCourses changed:', { 
    old: oldVal?.length, 
    new: newVal?.length,
    courses: newVal 
  });
...
async initializeCourses() {
  ...
  try {
    ...
    if (coursesData && coursesData.length > 0) {
      ...
      console.log('Displayed courses:', this.displayedCourses);
    } else {
...
updateDisplayedCourses() {
  ...
  console.log('Updating displayed courses:', this.displayedCourses.length);
},
