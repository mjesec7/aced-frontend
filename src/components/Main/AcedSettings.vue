    ...
    <main class="max-w-6xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="flex items-center gap-4 p-5 bg-white rounded-2xl shadow-sm">
          <div class="w-12 h-12 flex items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 text-white">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/>
            </svg>
          </div>
          <div>
            <p class="text-xs text-slate-500 uppercase tracking-wide font-medium">Current Plan</p>
            <h3 class="text-xl font-bold text-slate-900">{{ currentPlanLabel }}</h3>
            <span :class="[
              'inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase mt-1',
              currentPlan !== 'free' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
            ]">
        ...
        <!-- Messages Stat -->
        <div class="flex items-center gap-4 p-5 bg-white rounded-2xl shadow-sm">
          <div class="w-12 h-12 flex items-center justify-center rounded-xl bg-gradient-to-br from-teal-400 to-emerald-500 text-white">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
            </svg>
          </div>
          <div class="flex-1">
            <p class="text-xs text-slate-500 uppercase tracking-wide font-medium">Messages</p>
            <h3 class="text-xl font-bold text-slate-900">{{ currentUsageMessages }} <span class="text-sm font-normal text-slate-400">/ {{ usageLimitsMessages === -1 ? '∞' : usageLimitsMessages }}</span></h3>
            <div class="h-1 bg-slate-200 rounded-full mt-2 overflow-hidden">
              <div class="h-full bg-gradient-to-r from-indigo-500 to-purple-600 rounded-full transition-all" :style="{ width: messageUsagePercentage + '%' }"></div>
            </div>
          </div>
        </div>
        ...
        <!-- Images Stat -->
        <div class="flex items-center gap-4 p-5 bg-white rounded-2xl shadow-sm">
          <div class="w-12 h-12 flex items-center justify-center rounded-xl bg-gradient-to-br from-pink-500 to-orange-500 text-white">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div class="flex-1">
            <p class="text-xs text-slate-500 uppercase tracking-wide font-medium">Images</p>
            <h3 class="text-xl font-bold text-slate-900">{{ currentUsageImages }} <span class="text-sm font-normal text-slate-400">/ {{ usageLimitsImages === -1 ? '∞' : usageLimitsImages }}</span></h3>
            <div class="h-1 bg-slate-200 rounded-full mt-2 overflow-hidden">
              <div class="h-full bg-gradient-to-r from-indigo-500 to-purple-600 rounded-full transition-all" :style="{ width: imageUsagePercentage + '%' }"></div>
            </div>
          </div>
        </div>

        <!-- Days Remaining Stat -->
        <div v-if="currentPlan !== 'free' && subscriptionExpiryInfo" class="flex items-center gap-4 p-5 bg-white rounded-2xl shadow-sm">
          <div class="w-12 h-12 flex items-center justify-center rounded-xl bg-gradient-to-br from-sky-400 to-cyan-500 text-white">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div>
            <p class="text-xs text-slate-500 uppercase tracking-wide font-medium">Days Remaining</p>
            <h3 class="text-xl font-bold text-slate-900">{{ subscriptionExpiryInfo?.daysRemaining || 0 }}</h3>
            <span :class="[
              'inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase mt-1',
              subscriptionExpiryInfo?.isExpiring ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'
            ]">
        ...
        <div class="flex flex-col gap-6">
          ...
          <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div class="p-5">
              ...
              <div v-if="isEditingName" class="flex justify-end gap-3 pt-5 border-t border-slate-100">
                <button @click="cancelEditingName" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl transition-all">Cancel</button>
                <button @click="saveNameChanges" class="px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-semibold rounded-xl transition-all">Save Changes</button>
              </div>
            </div>
          </div>
          ...
          <div v-if="!isGoogleUser" class="bg-white rounded-2xl shadow-sm overflow-hidden">
            <div class="flex items-center gap-3 p-5 border-b border-slate-100">
              ...
              <div>
                ...
                <p class="text-xs text-slate-500">Manage password and security</p>
              </div>
            </div>
            ...
            <div class="p-5">
              <div class="mb-5">
                ...
                <input 
                  type="password" 
                  v-model="oldPassword" 
                  class="w-full px-4 py-3 bg-slate-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-xl text-sm text-slate-900 outline-none transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                  placeholder="••••••••"
                  :disabled="loading"
                />
              </div>
              ...
              <div class="mb-5">
                <label class="block text-xs font-medium text-slate-600 mb-2">Confirm New Password</label>
                <input 
                  type="password" 
              ...
              <div class="flex justify-between items-center pt-5 border-t border-slate-100">
                ...
                <button @click="saveChanges" class="px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-semibold rounded-xl transition-all disabled:opacity-60 disabled:cursor-not-allowed" :disabled="loading">
        ...
        <div class="flex flex-col gap-6">
          ...
          <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div class="p-5">
              ...
              <div class="text-center p-5 bg-slate-50 rounded-2xl mb-6">
                <div :class="[
                  'inline-block px-6 py-2 rounded-full text-lg font-bold uppercase tracking-wide',
                  currentPlan === 'free' ? 'bg-slate-200 text-slate-600' : 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white'
                ]">
                  {{ currentPlanLabel }}
                </div>
                <div v-if="currentPlan !== 'free' && subscriptionExpiryInfo" class="mt-3 text-sm text-slate-600">
                  <p>Expires: {{ subscriptionExpiryInfo.formattedDate }} <span class="text-slate-400">({{ subscriptionExpiryInfo.daysRemaining }} days left)</span></p>
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                <!-- 1 Month -->
                <div class="relative p-5 border-2 border-slate-200 hover:border-indigo-400 rounded-2xl transition-all hover:shadow-lg group">
                  <div class="text-center">
                    <h3 class="text-lg font-bold text-slate-900 mb-1">1 Month</h3>
                    <div class="text-2xl font-bold text-indigo-600 mb-3">250,000 <span class="text-sm font-normal text-slate-500">UZS</span></div>
                    <ul class="text-left text-xs text-slate-600 space-y-2 mb-4">
                    ...
                    <button 
                      ...
                      class="w-full py-2.5 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-semibold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                ...
                <!-- 3 Months -->
                <div class="relative p-5 border-2 border-amber-400 rounded-2xl shadow-lg bg-gradient-to-b from-amber-50 to-white">
                  <div class="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-gradient-to-r from-amber-400 to-orange-500 text-white text-[10px] font-bold uppercase rounded-full tracking-wide">
                    Popular
                  </div>
                  <div class="text-center pt-2">
                    <h3 class="text-lg font-bold text-slate-900 mb-1">3 Months</h3>
                    <div class="text-2xl font-bold text-indigo-600 mb-1">675,000 <span class="text-sm font-normal text-slate-500">UZS</span></div>
                    <div class="inline-block px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[10px] font-bold rounded-full mb-3">SAVE 10%</div>
                    <ul class="text-left text-xs text-slate-600 space-y-2 mb-4">
                      <li class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-emerald-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        Unlimited messages
                    ...
                    <button 
                      @click="goToUpgrade(3)" 
                      :disabled="currentPlan !== 'free'"
                      class="w-full py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-semibold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                ...
                <!-- 6 Months -->
                <div class="relative p-5 border-2 border-slate-200 hover:border-indigo-400 rounded-2xl transition-all hover:shadow-lg group">
                  <div class="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-[10px] font-bold uppercase rounded-full tracking-wide">
                    Best Deal
                  </div>
                  <div class="text-center pt-2">
                    <h3 class="text-lg font-bold text-slate-900 mb-1">6 Months</h3>
                    <div class="text-2xl font-bold text-indigo-600 mb-1">1,200,000 <span class="text-sm font-normal text-slate-500">UZS</span></div>
                    <div class="inline-block px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[10px] font-bold rounded-full mb-3">SAVE 20%</div>
                    <ul class="text-left text-xs text-slate-600 space-y-2 mb-4">
                      <li class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-emerald-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
              ...
              <div class="pt-5 border-t border-slate-100">
                <h4 class="text-sm font-semibold text-slate-900 mb-3">Have a promocode?</h4>
                <div class="flex gap-3">
                  ...
                  <button 
                    ...
                    class="px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div class="p-5">
              <div class="flex justify-between items-center py-4 border-b border-slate-100">
                ...
                <button @click="handleLogout" class="px-4 py-2 border-2 border-red-500 text-red-500 hover:bg-red-500 hover:text-white font-semibold rounded-xl transition-all">
                  Sign Out
                </button>
              </div>
              ...
              <div class="flex justify-between items-center py-4">
                ...
                <button @click="confirmDeleteAccount" class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-xl transition-all">
                  Delete Account
                </button>
              </div>
            </div>
...
export default {
  ...
  setup() {
    ...
    // Subscription
    const goToUpgrade = (months = 1) => {
      router.push({ path: '/upgrade', query: { duration: months } });
    };
